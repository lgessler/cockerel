...
import { QUERY } from 'src/components/Mixer/MixersCell'
    ...
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        ...
        <tbody>
          {mixers.map((mixer) => (
            <tr key={mixer.id}>
              <td>{truncate(mixer.id)}</td>
              <td>{truncate(mixer.name)}</td>
              <td>{truncate(mixer.url)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.mixer({ id: mixer.id })}
                    title={'Show mixer ' + mixer.id + ' detail'}
